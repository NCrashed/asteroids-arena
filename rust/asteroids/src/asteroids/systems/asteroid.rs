...
impl SysAsteroid {
    pub fn new(world: &mut World) -> Self {
        ...
        let reader_id = world
            .fetch_mut::<EventChannel<AsteroidBreak>>()
            .register_reader();
...
impl<'a> System<'a> for SysAsteroid {
    ...
    fn run(
        &mut self,
        (chan, mut audio_chan, entities, mut asteroid, mut mass, mut pos, mut vel, mut rot): Self::SystemData,
    ) {
...
fn spawn_asteroid<'a>(
    ...
    asteroid_store
        ...
        .stringify()?;
    mass_store
        .insert(e, Mass(PI * radius * radius * ASTEROID_DENSITY))
        .stringify()?;
